...
async function geocodeLocation(value) {
  ...
  try {
    ...
    console.warn("Regional geocode failed:", error);
  }
...
async function getRegionalCenter(weather) {
  if (weather && Number.isFinite(weather.lat) && Number.isFinite(weather.lon)) {
    return {
  ...
  if (
    weather &&
    weather.coords &&
    Number.isFinite(weather.coords.lat) &&
    Number.isFinite(weather.coords.lon)
  ) {
    return {
  ...
  const center = await geocodeLocation(getLocationText());
...
function createRegionalMap() {
  ...
  regionalMap = L.map(mapEl, {
    ...
    touchZoom: false
      ...
      L.tileLayer("https://api.maptiler.com/maps/019fe1fa-e820-78cd-bccf-a2a78e5f39b3/256/{z}/{x}/{y}.png?key=WurpHUdlwHNJ9yMlO1YM", {
    maxZoom: 19,
    minZoom: 3,
    noWrap: true,
    crossOrigin: true
...
function renderFrameImage() {
  ...
  frame.innerHTML =
    '<img id="regional-frame-img" data-edit-id="regional-frame-img" src="/bg/ws4000wherediditgo.png" alt="" />';
}
...
function renderDots(fallbackWeather, areaCities, token) {
  ...
  dots.innerHTML = areaCities
    .map(function (city) {
      ...
      if (point.x < 85 || point.x > 1010 || point.y < 220 || point.y > 735) {
        return "";
      }
      ...
      return (
        '<div class="regional-dot" data-edit-id="reg-dot-' + city.slug + '" style="left:' +
        Math.round(point.x) + 'px; top:' + Math.round(point.y) + 'px;">' +
